...
function HostSetup() {
  ...
  const netModes = GAME_MODES.filter((m) => m.maxPlayers > 1);
    ...
    <>
      <motion.div className="card" initial={{ scale: 0.96, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}>
        ...
        <div className="wrap" style={{ marginTop: 12, justifyContent: 'center' }}>
          ...
          <span className="chip">🗺️ {settings.boardSize}×{settings.boardSize}</span>
        </div>
...
function ClientSide() {
  ...
  return (
    <>
      ...
      <div className="waiting-hint">Ждём, когда хост начнёт бой…</div>
