...
public class LanPlugin extends Plugin {
    ...
    private volatile int advertisePort = 45631;
    ...
    public void startServer(PluginCall call) {
        int port = call.getInt("port", 45630);
        if (server != null) {
            JSObject result = new JSObject();
            result.put("ip", localIpAddress());
    ...
    public void startAdvertise(PluginCall call) {
        ...
        advertisePort = call.getInt("port", 45631);
        if (advertising) {
            call.resolve();
            return;
        }
    ...
    public void startDiscovery(PluginCall call) {
        int port = call.getInt("port", 45631);
        if (discovering) {
            call.resolve();
            return;
        }
